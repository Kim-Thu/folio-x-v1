document.querySelectorAll<HTMLElement>("[data-entry-index]").forEach((root) => {
	...
	showAll?.addEventListener("click", () => {
		root.querySelectorAll<HTMLElement>("[data-entry-row][hidden]").forEach((row) => {
			row.hidden = false;
		});
	...
	select.addEventListener("change", () => {
		const rows = Array.from(root.querySelectorAll<HTMLElement>("[data-entry-row]"));
		const footer = root.querySelector<HTMLElement>("[data-entry-footer]");
		const direction = select.value === "oldest" ? 1 : -1;
		rows
			.sort((left, right) =>
				direction * (Number(left.dataset.order) - Number(right.dataset.order)),
			)
			.forEach((row) => {
