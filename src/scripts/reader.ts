export function initReader(scope: ParentNode = document): void {
	scope.querySelectorAll<HTMLElement>("[data-reader-root]").forEach((root) => {
		const scaleToggle = root.querySelector<HTMLButtonElement>(
			"[data-reader-scale-toggle]",
		);
		...
		const reactions = root.querySelectorAll<HTMLButtonElement>(
			"[data-reader-reaction]",
		);
		...
		bookmark?.addEventListener("click", () => {
			bookmark.setAttribute(
				"aria-pressed",
				String(bookmark.getAttribute("aria-pressed") !== "true"),
			);
		});
