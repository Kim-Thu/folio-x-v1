...
function selectRandomTip(tips: string[]): string | undefined {
	...
	try {
		...
		const availableTips =
			tips.length > 1 ? tips.filter((tip) => tip !== previousTip) : tips;
		const selectedTip =
...
export function initLoadingScreen(): void {
	...
	const progressValue = progress?.querySelector<HTMLElement>("[data-progress-value]");
	const progressNumber = screen?.querySelector<HTMLElement>(
		"[data-loading-progress-number]",
	);
