...
export default function CSliderEmbla({
	...
	useEffect(() => {
		...
		const root = viewport.closest<HTMLElement>("[data-slider]");
		if (!root) return;
		...
		const totalLabel = root.querySelector<HTMLElement>("[data-slider-total]");
		const directButtons = Array.from(
			root.querySelectorAll<HTMLButtonElement>("[data-slider-go]"),
		);
